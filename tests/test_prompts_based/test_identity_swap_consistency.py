"""Tests for IdentitySwapConsistency."""
...
from bias_scope.prompts_based.identity_swap_consistency import (
...
def _make_metric():
    """Create IdentitySwapConsistency without dataset setup at init time."""
    return IdentitySwapConsistency(model_name="test-model", api_key="test-key")
...
def mock_embedder():
    ...
    with patch(
        "bias_scope.prompts_based.identity_swap_consistency.SentenceTransformer"
    ) as mock_cls:
        instance = MagicMock()
...
def mock_completion():
    ...
    with patch("bias_scope.prompts_based.identity_swap_consistency.completion") as m:
        mock_resp = MagicMock()
...
@pytest.fixture
def metric():
    """Create IdentitySwapConsistency fixture."""
    return _make_metric()


class TestIdentitySwapConsistency:
    """Test suite for IdentitySwapConsistency."""

    def test_basic_functionality(self, metric, mock_completion, mock_embedder):
        """Result has correct keys and types."""
        with patch("bias_scope.prompts_based.identity_swap_consistency.load_dataset") as m:
    ...
    def test_empty_dataset_raises_error(self, mock_embedder, mock_completion):
        ...
        with patch(
            "bias_scope.prompts_based.identity_swap_consistency.load_dataset"
        ) as m:
            m.return_value = []
            metric = IdentitySwapConsistency(model_name="test", api_key="test")
