...
| SentenceBiasScore (Dolci et al. 2023) | — (blocked, not excluded) | Paywalled with no preprint, so the Section 4.0 gate cannot be passed. Carries the `unaudited` status, which is a statement about our evidence, not about the metric. REVIEW_LATER RL-029. |
