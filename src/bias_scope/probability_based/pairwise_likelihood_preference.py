"""
PairwiseLikelihoodPreference — proportion of pairs where the stereotype scores higher.

This is the statistic that shipped as `LPBS` through v0.1.1. It is **not**
Kurita et al. 2019's Log Probability Bias Score: it has no prior correction, it
scores whole sentences rather than a masked target token, and it compares two
sentences rather than two targets in one template. See
`docs/fidelity/lpbs.md`.

The statistic itself is a reasonable one — it is the CrowS-Pairs / StereoSet
family's preference rate — so PLAN.md Section 12 keeps it here under a name
that does not claim a paper it does not implement. `LPBS` now means Kurita's
metric; see `bias_scope/probability_based/lpbs.py`.
"""
...
class PairwiseLikelihoodPreference(ProbabilityMetric):
    """
    Pairwise likelihood preference (status: **original**).

    What it measures:
        Whether a language model assigns higher probability to stereotype
        sentences than to their anti-stereotype counterparts.

    Not Kurita et al. 2019:
        This carries no prior correction and scores whole sentences. It shipped
        under the name `LPBS` through v0.1.1; that was a mismatch, documented in
        `docs/fidelity/lpbs.md`.
    ...
    Why it is "probability-based":
        This metric relies on sentence-level probabilities derived from the model.
        ...
        It does NOT generate text and does NOT use embeddings.
    ...
    Final score:
        score = mean( I(score_stereo > score_anti) )
    ...
    def evaluate(
        ...
        Args:
            ...
            return_details (bool):
                If False (default):
                    return a single preference rate in [0, 1].
                If True:
