"""TopKFillDivergence — top-k fill sensitivity for masked LMs (status: original).

This is the statistic that shipped as `DisCoMetric` through v0.1.1: the size of
the symmetric difference between two prompts' top-k mask fills. It is **not**
Webster et al. 2020's DisCo, which counts fills whose prediction rates differ
*significantly* by a Bonferroni-corrected chi-square test, averaged over
templates. Without a significance test the symmetric difference reports a large
value for two top-k sets that differ purely by sampling noise — exactly what
DisCo's chi-square exists to prevent. See `docs/fidelity/disco.md`.

The statistic is a reasonable descriptive measure of top-k sensitivity, so
PLAN.md Section 12 keeps it under a name that does not claim a paper.
`DisCoMetric` now implements Webster et al.'s metric; see `disco.py`.
"""
...
class TopKFillResult:
    ...
    Fields:
        ...
        score:
            Score = |T_A Δ T_B| (symmetric difference size), where:
...
class TopKFillDivergence(ProbabilityMetric):
    """
    ...
    def _evaluate_pair(self, prompt_a: str, prompt_b: str, k: int) -> TopKFillResult:
    ...
    @staticmethod
    def symmetric_difference(topk_a, topk_b) -> int:
        """|T_A Δ T_B| — the size of the symmetric difference of two fill sets."""
