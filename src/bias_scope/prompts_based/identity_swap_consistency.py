"""IdentitySwapConsistency — response stability under identity substitution.

Status: **original**. Renamed from `CounterfactualFairness` in 0.2.0.

The old name was the exact title of Kusner et al. 2017, which defines a
*causal criterion on a predictor* — a predictor is counterfactually fair if
its distribution is unchanged in the counterfactual world where a protected
attribute differs, given a causal model. That is a definition, not a score,
and it needs a causal graph. This metric measures cosine similarity between
response embeddings under identity swaps: the same intuition, a different
thing. See `docs/fidelity/originals.md`.
"""
...
class IdentitySwapConsistency(PromptBasedMetric):
    """
    ...
    def evaluate(  # noqa: C901 (RL-002)
        ...
        Examples:
            ...
            >>> assert 0 <= r["avg_similarity"] <= 1
            >>> assert abs(r["bias_score"] - (1 - r["avg_similarity"])) < 1e-6
        """
