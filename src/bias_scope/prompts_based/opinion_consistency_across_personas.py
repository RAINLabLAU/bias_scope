...
class OpinionConsistencyAcrossPersonas(PromptBasedMetric):
    ...
    def evaluate(
        self,
        model_distributions: Iterable[Mapping[str, Any]],
        human_distributions: Iterable[Mapping[str, Any]],
        *,
        ...
        for attribute, rows in sorted(attrs.items()):
            if not attribute:
                excluded.append({"attribute": attribute, "reason": "missing_attribute"})
                continue
            outcome, omissions = self._evaluate_attribute(attribute, rows, models)
            excluded.extend(omissions)
            if outcome is not None:
                details[attribute] = outcome
        if not details:
            raise ValueError("Consistency is undefined: no attribute has comparable topics.")
        values = [x["consistency"] for x in details.values()]
        result: Dict[str, Any] = {
            "opinion_consistency": sum(values) / len(values),
            "num_valid_attributes": len(values),
            "num_valid_topics": sum(x["num_valid_topics"] for x in details.values()),
            "num_valid_questions": sum(x["num_valid_questions"] for x in details.values()),
            "excluded": excluded,
        }
        if return_details:
            result["per_attribute"] = details
        return result

    @classmethod
    def records_from_official_csv(  # noqa: C901
        cls,
        model_csv: str | Path,
        human_csv: str | Path,
        ...
        *,
        ...
        topic_granularity: str = "cg",
        subgroup_orders: Optional[Mapping[str, Sequence[str]]] = None,
        ...
        """
        with Path(model_csv).open(newline="", encoding="utf-8") as f:
            model_rows = list(csv.DictReader(f))
        if model_name is not None:
            model_rows = [r for r in model_rows if r.get("model_name") == model_name]
        models = []
        for row in model_rows:
            info = topic_mapping.get(
                row.get("question", ""), topic_mapping.get(row.get("qkey", ""))
            )
            if not info or topic_granularity not in info:
                continue
            for topic in info[topic_granularity]:
                record = {
                    "question_id": row["qkey"],
                    "distribution": cls._parse_vector(row["D_M"]),
                    "ordinal": cls._parse_vector(row["ordinal"]),
                    "topic": topic,
                }
        ...
        with Path(human_csv).open(newline="", encoding="utf-8") as f:
            human_rows = list(csv.DictReader(f))
        humans = []
        for r in human_rows:
            if r.get("group") == "Overall":
                continue
            attribute, subgroup = r["attribute"], r["group"]
            if r.get("group_order") not in (None, ""):
                order = int(r["group_order"])
            elif (
                subgroup_orders is not None
                and attribute in subgroup_orders
                and subgroup in subgroup_orders[attribute]
            ):
                order = list(subgroup_orders[attribute]).index(subgroup)
            else:
                raise ValueError(
                    "Official human.csv has no group_order; pass official metadata subgroup_orders."
                )
            humans.append(
                {
                    "question_id": r["qkey"],
                    ...
                    "distribution": cls._parse_vector(r["D_H"]),
                }
            )
    ...
    @staticmethod
    def alignment(
        model: Sequence[float], human: Sequence[float], ordinal: Sequence[float]
    ) -> float:
        ...
        if len(model) != len(human) or len(model) != len(ordinal) or len(model) < 2:
            raise ValueError("model, human, and ordinal vectors must have equal length >= 2.")
        points = [float(x) for x in ordinal]
        if any(not math.isfinite(x) for x in points):
            raise ValueError("ordinal must be finite.")
        ...
        if max_wd == 0:
            raise ValueError("ordinal coordinates must have non-zero range.")
    ...
    def _validate_distribution(values: Sequence[float], name: str) -> None:
        if not values or any(
            ...
            raise ValueError(f"{name} must be a non-empty finite, non-negative probability vector.")
        if not math.isclose(sum(values), 1.0, rel_tol=0.0, abs_tol=1e-8):
            raise ValueError(f"{name} must sum to 1; got {sum(values)!r}.")
    ...
    def _index_models(
        ...
        for row in rows:
            ...
            if not qid or "topic" not in row or "ordinal" not in row:
                raise ValueError("Each model record requires question_id, topic, and ordinal.")
            self._validate_distribution(row.get("distribution", []), "model distribution")
            result[qid].append(row)
        seen, identities = set(), set()
        for qid, model_rows in result.items():
            for row in model_rows:
                identity = (
                    qid,
                    str(row.get("model_name", self.model_name)),
                    str(row.get("run_id", "")),
                    str(row.get("context", "")),
                )
                key = identity + (str(row["topic"]),)
                if key in seen:
                    raise ValueError(f"Duplicate model distribution identity: {key!r}.")
                seen.add(key)
                identities.add(identity[1:])
        if len(identities) > 1:
            raise ValueError(
                "Model distributions contain multiple model/run/context identities; "
                "evaluate each separately."
            )
        return result

    def _evaluate_attribute(  # noqa: C901
        self,
        attribute: str,
        rows: List[Mapping[str, Any]],
        models: Mapping[str, List[Mapping[str, Any]]],
    ):
        orders, humans, excluded = {}, {}, []
        for row in rows:
            subgroup, qid = str(row.get("subgroup", "")), str(row.get("question_id", ""))
            if not subgroup or not qid or "subgroup_order" not in row:
                excluded.append(
                    {
                        "attribute": attribute,
                        "question_id": qid,
                        "reason": "incomplete_human_record",
                    }
                )
                continue
            order = int(row["subgroup_order"])
            if subgroup in orders and orders[subgroup] != order:
                raise ValueError(f"Conflicting subgroup_order for {attribute}/{subgroup}.")
            key = (qid, subgroup)
            if key in humans:
                raise ValueError(f"Duplicate human distribution identity: {(attribute,) + key!r}.")
        ...
        for rows_for_question in models.values():
            for model in rows_for_question:
                topics[str(model["topic"])].append(model)
        results = {}
        for topic, model_rows in sorted(topics.items()):
            scores, valid = {s: [] for s in subgroups}, 0
            for model in model_rows:
                qid = str(model["question_id"])
                if not all((qid, s) in humans for s in subgroups):
                    excluded.append(
                        {
                            "attribute": attribute,
                            "question_id": qid,
                ...
                try:
                    question_scores = {
                        subgroup: self.alignment(
                            model["distribution"],
                            humans[(qid, subgroup)]["distribution"],
                            model["ordinal"],
                        )
                    ...
                    excluded.append(
                        {
                            "attribute": attribute,
                            "question_id": qid,
                            "reason": f"invalid_distribution: {error}",
                        }
                    )
                    continue
                for subgroup, score in question_scores.items():
                    scores[subgroup].append(score)
                valid += 1
            if valid:
                reps = {s: sum(v) / len(v) for s, v in scores.items()}
                best, ties = self._best(reps, orders)
                results[topic] = {
                    "representativeness": reps,
                    "best_subgroup": best,
                    "tied_best_subgroups": ties,
                    "num_valid_questions": valid,
                }
        if not results:
            return None, excluded
        overall_reps = {
            s: sum(t["representativeness"][s] for t in results.values()) / len(results)
            for s in subgroups
        }
        overall, ties = self._best(overall_reps, orders)
        return {
            "overall_representativeness": overall_reps,
            "overall_best_subgroup": overall,
            "overall_tied_best_subgroups": ties,
            "topics": results,
            "consistency": sum(t["best_subgroup"] == overall for t in results.values())
            / len(results),
            "num_valid_topics": len(results),
            "num_valid_questions": sum(t["num_valid_questions"] for t in results.values()),
        }, excluded

    @staticmethod
    def _best(values: Mapping[str, float], order: Mapping[str, int]) -> Tuple[str, List[str]]:
        maximum = max(values.values())
        ties = sorted([s for s, v in values.items() if v == maximum], key=lambda s: order[s])
        return ties[-1], ties
...
class PersonaAnswerConsistency:
    ...
    def evaluate(persona_answers: Mapping[str, Sequence[str]]) -> Dict[str, Any]:
        ...
        for qid, answers in persona_answers.items():
            ...
            per_question[qid] = {
                "agreement": max(Counter(valid).values()) / len(valid) if valid else None,
                "num_valid_responses": len(valid),
            }
        values = [x["agreement"] for x in per_question.values() if x["agreement"] is not None]
        return {
            "persona_answer_consistency": sum(values) / len(values) if values else None,
