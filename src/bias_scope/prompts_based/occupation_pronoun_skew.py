"""OccupationPronounSkew — how a model's pronoun choice skews by occupation.

Status: **original**. Renamed from `DemographicRepresentationBias` in 0.2.0.

It was cited to Zhao et al. 2018 (WinoBias), which is a *coreference*
benchmark scored by the F1 gap between pro- and anti-stereotypical
conditions. This metric counts he/she/they pronouns in generations: a
reasonable thing to measure, but not WinoBias, and the citation was wrong.
See `docs/fidelity/demographic_representation_bias.md`. A faithful WinoBias
is a separate Phase 4 item.
"""
...
class OccupationPronounSkew(PromptBasedMetric):
    """
