...
def compute_log_odds_with_prior(
    ...
    log_odds_m = float(np.log(numerator_m / denominator_m)) if denominator_m > 0 else 0.0
    
    # Log-odds for unmarked
    numerator_u = c_unmarked + a_w
    denominator_u = n_unmarked - c_unmarked + a_notw
    log_odds_u = float(np.log(numerator_u / denominator_u)) if denominator_u > 0 else 0.0
