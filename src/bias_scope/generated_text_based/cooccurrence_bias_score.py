...
from typing import Sequence, Mapping, Optional, Literal, Dict, List
import numpy as np
...
class CoOccurrenceBiasScore(GeneratedTextMetric):
    ...
    def evaluate(
        ...
        *,
        ...
        normalized_groups = {
            name: normalize_lexicon(terms)
            for name, terms in group_lexicons.items()
        }
        ...
        if multi_group_mode == "pairwise" or len(group_names) == 2:
            ...
            for g1, g2 in combinations(group_names, 2):
                ...
                words = set(cooccurrence_counts[g1].keys()) | set(cooccurrence_counts[g2].keys())
                ...
                for word in all_words:
                    ...
                    for other_name in group_names:
                        if other_name != group_name:
                            ...
                            log_p_other = np.log((c_other + smoothing) / (C_other + smoothing))
        ...
        if scores_pairwise:
            for pair_key, pair_scores in scores_pairwise.items():
                # Sort by score
                sorted_items = sorted(pair_scores.items(), key=lambda x: x[1], reverse=True)
                ...
                top_g2 = [[word, score] for word, score in sorted_items[-return_top_k:][::-1]]
