"""StereotypeRuleHitRate — rule-based stereotype detection (status: original).

This is the statistic that shipped as `StereotypicalAssociations` through
v0.1.1. It is **not** HELM's metric of that name: HELM defines the mean over
target words of a TVD between group co-occurrence counts and uniform, with no
notion of user-supplied rules, windows, or hit rates. See
`docs/fidelity/stereotypical_associations.md`.

The rule matcher is a useful thing in its own right — it answers "how often does
this specific stereotype surface?" — so PLAN.md Section 12 keeps it under a name
that does not claim a paper. `StereotypicalAssociations` now implements HELM's
metric; see `stereotypical_associations.py`.
"""
...
class StereotypeRuleHitRate(GeneratedTextMetric):
    """
    ...
    def evaluate(  # noqa: C901 (RL-002)
        ...
        *,
        ...
        if matcher not in {"token_window", "regex"}:
            raise ValueError(
                f"matcher must be 'token_window' or 'regex', got '{matcher}'"
            )
        ...
        for i, rule in enumerate(stereotype_rules):
            ...
            if matcher == "token_window":
                ...
                group_terms = self._validate_texts(
                    rule["group_terms"], f"rule '{rule_name}' group_terms"
                )
                attribute_terms = self._validate_texts(
                    rule["attribute_terms"], f"rule '{rule_name}' attribute_terms"
                )
                ...
                processed_rules.append(
                    {"name": rule_name, "type": "regex", "pattern": compiled_pattern}
                )
        ...
        for text in generations_list:
            ...
            per_generation.append(
                {"any_hit": generation_any_hit, "hits": generation_hits}
            )
        ...
        for rule in processed_rules:
            ...
            rules_results.append(
                {"name": rule_name, "hits": hits, "rate_per_1k": rate_per_1k}
            )
