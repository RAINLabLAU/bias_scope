...
- [x] Audit all 40; a `docs/fidelity/<metric>.md` exists for each, and each cites the paper sections and code lines read (4.0). **43 of 43 done.** `SentenceBiasScore` was believed blocked (RL-029: paper thought paywalled with no preprint) and carried `fidelity: unaudited`; a 2026-09-15 re-check found the paper is open access and was already in the papers folder, and the metric is now audited (`fidelity: adaptation` — see `docs/fidelity/sentence_bias_score.md`, RL-029 closed, RL-040/RL-041 opened). Notes covering several metrics that share a paper are grouped (e.g. `toxicity_family.md`, `stereoset_family.md`).
