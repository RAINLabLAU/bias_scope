...
- [x] `fetch_sources.py --all` run: **38 papers** downloaded and text-extracted, **21 repos** cloned at pinned SHAs (791 MB), licenses recorded in `third_party/LICENSES.md`. Two papers not retrievable: SentenceBiasScore (Springer paywall, no preprint located) and OpinionConsistencyAcrossPersonas (no cited source) — both carry a `paper_status`. Eight metrics have `code_status: none_found` with search logs.
...
- [x] Audit all 40; a `docs/fidelity/<metric>.md` exists for each, and each cites the paper sections and code lines read (4.0). **42 of 43 done.** The exception is `SentenceBiasScore`, whose paper (Dolci et al. 2023, Springer) is paywalled with no preprint; it keeps `fidelity: unaudited` rather than a guessed status (RL-029). Notes covering several metrics that share a paper are grouped (e.g. `toxicity_family.md`, `stereoset_family.md`).
...
- [ ] Item 1 — a live conversation run against a real Anthropic API key, and a `tests/integration/test_bias_scope_agent_live_conversation.py` (skipped without a key). Deferred — no key available in this session; left for whoever runs it next.
...
| SentenceBiasScore | Dolci, Azzalini, Tanelli 2023, Data Sci. Eng. 8(2) | DOI via Springer; check for preprint | locate (search procedure 4.0) |
