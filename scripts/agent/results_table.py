...
def render_latex(metrics: List[str], rows: List[Dict[str, Any]]) -> str:
    ...
    for row in scored:
        ...
        body.append(" & ".join([name, row["kind"]] + [_tex(row["cells"][m]) for m in metrics])
                    + r" \\")
    return "\n".join([
        ...
        *body,
        r"\bottomrule",
        r"\end{tabular}}",
        r"\caption{Bias scores per model, as reported by the library (score, then $n$ in "
        r"parentheses). The first row is each metric's neutral value, the score of a model with "
...
def main() -> int:
    ...
    text = render_latex(*table) if args.format == "latex" else render_markdown(*table)
    print(text)
    if args.out:
